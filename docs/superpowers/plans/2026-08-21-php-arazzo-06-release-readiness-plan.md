...
- [ ] Update root imports to actual `Runner` and `Resolver` namespaces.
...
- [ ] Commit `docs: update PHP Arazzo execution and compatibility guide`.

### Task 2: Normalize package metadata and release artifacts

- [ ] Verify package names, versions, runtime dependencies, PSR requirements, autoload rules, licenses, and package descriptions.
- [ ] Confirm development-only dependencies do not appear in runtime requirements.
- [ ] Add changelog entries for the breaking execution-core migration and OpenAPI executor changes.
...
- [ ] Commit `chore: prepare PHP packages for clean installation`.
...
- [ ] Run `make verify` or its individual equivalent locally; record missing external prerequisites rather than weakening gates.
- [ ] Commit `ci: align local and GitHub verification gates`.
...
- [ ] Run the full CI-equivalent command set.
- [ ] Commit `docs: add PHP Arazzo release checklist`.
